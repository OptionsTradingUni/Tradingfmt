  ...
  ({ username, avatarColor, message, timestamp, reactions = [], verified = false, channelName = "profits", embeddedImageUrl }, ref) => {
    ...
    return (
      <div ref={ref} className="bg-[#313338] w-[669px] font-sans overflow-hidden">
        {/* Discord Header */}
        <div className="bg-[#313338] border-b border-[#26282C] px-4 py-3 flex items-center justify-between">
        ...
        <div className="px-4 py-3">
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[#DBDEE1] text-[15px] leading-[1.375] mb-2">
                {renderMessage()}
              </div>
              {embeddedImageUrl && (
              ...
              {reactions.length > 0 && (
                <div className="flex gap-1 mt-2">
                  {reactions.map((reaction, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-1.5 bg-[#2B2D31] hover:bg-[#35373C] border border-[#3F4147] rounded px-2 py-1 cursor-pointer transition-colors"
                    >
